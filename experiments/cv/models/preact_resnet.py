# https://github.com/tml-epfl/understanding-sam/blob/main/deep_nets/models.py
...
class PreActResNet(nn.Module):
    def __init__(
        ...
        self.layer4 = self._make_layer(
            block, 8 * model_width, num_blocks[3], 2, droprate
        )
        self.bn = (
            nn.BatchNorm2d(8 * model_width * block.expansion)
            if self.bn_flag
            else nn.GroupNorm(self.gn_groups, 8 * model_width * block.expansion)
        )
        self.linear = nn.Linear(
            8 * model_width * block.expansion, 1 if n_cls == 2 else n_cls
        )
