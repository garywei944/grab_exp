...
def parse_args():
    ...
    # Sanity checks
    assert args.per_device_train_batch_size == 1, "GraB requires per-example gradient"
...
def main():
    ...
    send_example_telemetry("run_mlm_no_trainer.py", args)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                sampler.step(
                    {k: p.grad for k, p in model.named_parameters()}, batch=False
                )
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
