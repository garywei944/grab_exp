...
def parse_args():
    ...
    parser.add_argument(
        "--balance_type", type=str, default="rr", choices=[e.value for e in BalanceType]
    )
    ...
    parser.add_argument("--toy", action="store_true", help="use only 1% dataset")
    parser.add_argument(
        "--depth", type=int, default=5, help="depth of the GraB recursive balance tree"
    )
...
def main():
    ...
    send_example_telemetry("grab-lora-glue", args)
    ...
    # Gary: only use a subset of the data
    if args.toy:
        for k, v in raw_datasets.items():
            raw_datasets[k] = v.shard(index=0, num_shards=100)
    ...
    optimizer = torchopt.adamw(
        lr=torchopt.schedule.linear_schedule(
            ...
            end_value=0.0,
            transition_steps=args.max_train_steps,
        ),
        weight_decay=args.weight_decay,
    )
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers(f"grab-lora-{args.task_name}", experiment_config)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if (
            ...
            raise ValueError(
                "GraB currently doesn't support resuming from a checkpoint."
            )
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if args.with_tracking:
                total_loss += batch_loss.sum().float()
            ...
            if completed_steps % args.logging_steps == 0:
                logging.info(
                    ...
                    f"loss: {total_loss / completed_steps}"
                )
