...
def main():
    ...
    # make train dataset length divisible by batch size
    train_dataset = train_dataset.select(
        range(len(train_dataset) - len(train_dataset) % training_args.train_batch_size)
    )
    ...
    max_steps = epochs * len(train_loader) * 2
